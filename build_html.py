  ...
  <section class="view-gallery" id="viewGallery">
    ...
    <div id="adminGalleryActions" style="display:none; justify-content:space-between; align-items:center; background:#1e293b; padding:12px 18px; border-radius:10px; border:1px solid #334155;">
      ...
      <div style="display:flex; gap:8px; flex-wrap:wrap;">
        <button type="button" class="btn btn-outline btn-sm" id="btnAdminRestoreDefaults" title="استعادة القوالب الرسمية الأربعة">
          🔄 استعادة الأصلية
        </button>
      ...
      <div class="panel-section" id="adminBannerSec" style="display:none;">
        <div class="admin-action-banner">
          <div style="font-size:0.85rem; font-weight:800; color:#c084fc; display:flex; align-items:center; gap:6px;">
            ...
            لوحة الإدارة: تحرير ونشر هذا القالب للطلاب
          </div>
          <p style="font-size:0.78rem; color:#cbd5e1; line-height:1.5;">
            يمكنك كمسؤول تعديل رقم المشروع، اسمه، المشرف، الشعار، والأعمدة ونشر القالب ليظهر لجميع الطلاب.
          </p>
          <div style="display:flex; gap:8px;">
            <button type="button" class="btn btn-admin btn-sm" id="btnPublishTemplate" style="flex:1;">
            ...
            <div class="logo-preview-row">
              ...
              <div class="logo-buttons">
                ...
                <div style="display: flex; gap: 6px;">
                  ...
                  <button type="button" class="btn btn-outline btn-sm" id="btnResetLogo" title="استعادة شعار طيبة الأصلي">
                    استعادة
                  </button>
                </div>
              </div>
    ...
    <main class="canvas-area" id="canvasArea">
      ...
      <div class="student-top-guide-bar no-print" id="studentTopGuideBar">
        <div class="guide-bar-content">
          <div class="guide-bar-icon-wrap">💡</div>
          <div>
  ...
  <div class="modal-backdrop" id="printModal">
    <div class="modal-card">
      ...
      <div class="print-tip-item">
        ...
        <div><strong>رسومات الخلفية (Background graphics):</strong> تأكد من <strong>تفعيلها ✓</strong> لظهور ألوان العناوين والإطارات.</div>
      </div>
  ...
  <script>
          ...
          if (currentRole === 'admin') {{
            grid.innerHTML = `
              <div style="grid-column: 1/-1; text-align: center; padding: 50px 20px; background: rgba(15, 23, 42, 0.7); border: 2px dashed #475569; border-radius: 16px;">
                ...
                <h3 style="font-size: 1.25rem; font-weight: 800; color: #fff; margin-bottom: 8px;">لا توجد أي قوالب منشورة حالياً</h3>
                <p style="font-size: 0.88rem; color: #94a3b8; max-width: 480px; margin: 0 auto 16px auto; line-height: 1.6;">
                  تم حذف جميع القوالب. يمكنك كمسؤول إنشاء قالب استمارة جديد الآن ونشره فوراً للطلاب، أو استعادة النماذج الافتراضية.
                </p>
                <div style="display:flex; gap:10px; justify-content:center; flex-wrap:wrap;">
                  <button type="button" class="btn btn-admin" onclick="document.getElementById('btnAdminCreateNew').click()">
                    + إنشاء قالب جديد ونشره
                  </button>
                  <button type="button" class="btn btn-outline" id="btnRestoreInEmpty">
                    🔄 استعادة القوالب الأصلية
                  </button>
                </div>
              </div>
            `;
            setTimeout(() => {{
              const rBtn = document.getElementById('btnRestoreInEmpty');
              if (rBtn) {{
                rBtn.onclick = () => {{
                  templates = JSON.parse(JSON.stringify(INITIAL_TEMPLATES));
                  saveTemplates();
                  renderGallery();
                  showSwalToast('تمت الاستعادة!', 'تمت استعادة قوالب معاهد طيبة الافتراضية بنجاح.');
                }};
              }}
            }}, 10);
          }} else {{
        ...
        card.innerHTML = `
          ...
          <div class="template-card-footer">
            <button type="button" class="btn btn-primary" style="flex:1;" onclick="selectTemplate('${{tmpl.id}}')">
              ...
              <span>${{currentRole === 'admin' ? 'تعديل ونشر القالب' : 'اختيار هذه الاستمارة وتعبئتها'}}</span>
            </button>
            ${{currentRole === 'admin' ? `
              <button type="button" class="btn btn-outline btn-sm" onclick="event.stopPropagation(); deleteTemplate('${{tmpl.id}}');" title="حذف هذا القالب">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"></path></svg>
              </button>
            ` : ''}}
          </div>
        `;
    ...
    window.deleteTemplate = function(templateId) {{
      ...
      showSwalConfirm({{
        ...
        desc: `هل أنت متأكد من حذف <strong>«${{title}}»</strong> نهائياً من قائمة القوالب؟<br><span class="warn-highlight">⚠️ لن يتمكن الطلاب من الوصول إلى هذا القالب بعد حذفه!</span>`,
    ...
    function init() {{
      ...
      // Admin Publish Current Template
      document.getElementById('btnPublishTemplate').addEventListener('click', () => {{
        if (!currentTemplate) return;
        ...
        saveTemplates();
        showSwalToast('تم النشر بنجاح! 🎉', 'تم حفظ القالب ونشره لجميع الطلاب في قائمة الاستمارات.');
      }});
      ...
      document.getElementById('btnResetColumns').addEventListener('click', () => {{
        showSwalConfirm({{
          ...
          desc: 'هل تريد إعادة أعمدة الجدول إلى الأعمدة الرسمية الأصلية للمعهد؟<br><span class="warn-highlight">⚠️ سيتم استعادة الأعمدة الأربعة الافتراضية.</span>',
          confirmText: 'نعم، استعادة الأعمدة',
          cancelText: 'إلغاء',
          onConfirm: () => {{
            ...
            renderColumnsList();
            renderPreview();
            showSwalToast('تمت الاستعادة!', 'تمت إعادة الأعمدة إلى التوزيع الافتراضي.');
          }}
        }});
      }});
      ...
      document.getElementById('btnClearData').addEventListener('click', () => {{
        showSwalConfirm({{
          ...
          desc: 'سيتم مسح جميع البيانات والأسماء التي أدخلتها في جدول الطلاب بالكامل.<br><span class="warn-highlight">⚠️ لن تتمكن من التراجع عن هذه الخطوة بعد التأكيد!</span>',
