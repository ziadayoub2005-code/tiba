...
<body>
  ...
  <script>
      ...
      filtered.forEach(tmpl => {{
        ...
        card.innerHTML = `
          ...
          <div class="template-card-footer">
            ...
            ${{currentRole === 'admin' && templates.length > 1 ? `
    ...
    function setZoom(zoom) {{
      currentZoom = Math.min(Math.max(zoom, 0.25), 1.5);
      ...
      const scaledHeight = baseHeightPx * currentZoom;
    ...
    function fitToScreen() {{
      ...
      const containerWidth = container.clientWidth - (window.innerWidth <= 768 ? 24 : 60);
      const paperPxWidth = 210 * 3.7795275591;
      let fit = containerWidth / paperPxWidth;
      
      fit = Math.min(Math.max(fit, 0.28), 1.15);
      setZoom(fit);
    }}
