...
<body>
  ...
  <script>
        ...
        card.innerHTML = `
          ...
          <div class="template-card-footer">
            <button type="button" class="btn btn-primary" style="flex:1;" onclick="selectTemplate('${{tmpl.id}}')">
              ...
              <span>${{currentRole === 'admin' ? 'تعديل بيانات القالب' : 'اختيار هذه الاستمارة وتعبئتها'}}</span>
            </button>
            ${{currentRole === 'admin' ? `
              <button type="button" class="btn btn-outline btn-sm" onclick="event.stopPropagation(); togglePublishTemplate('${{tmpl.id}}');" title="${{isPublished ? 'إيقاف نشر هذا القالب للطلاب (تحويل لمسودة)' : 'نشر هذا القالب للطلاب الآن'}}" style="${{isPublished ? 'color:#f59e0b; border-color:rgba(245,158,11,0.4);' : 'color:#10b981; border-color:rgba(16,185,129,0.4);'}}">
                ${{isPublished ? `
    ...
    // Toggle Publish Status for a Template
    window.togglePublishTemplate = function(templateId) {{
      ...
      renderGallery();
      if (tmpl.published) {{
        showSwalToast('تم نشر القالب للطلاب!', 'أصبح هذا القالب متاحاً وظاهراً لجميع الطلاب الآن في قائمة الاستمارات.');
      }} else {{
        showSwalToast('تم إيقاف النشر!', 'تم تحويل القالب إلى مسودة خاصة ولن يظهر للطلاب.');
      }}
    }};
    ...
    function updateAdminBannerUI() {{
      ...
      if (isPub) {{
        ...
        badge.innerHTML = `
          ...
          منشور للطلاب
        `;
        desc.innerText = 'هذا القالب منشور حالياً ومتاح لكافة الطلاب في قائمة الاستمارات الرسمية.';
        toggleBtn.className = 'btn btn-outline btn-sm';
        toggleBtn.style.color = '#f59e0b';
        toggleBtn.style.borderColor = 'rgba(245,158,11,0.5)';
        toggleBtn.innerHTML = `
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="10" y1="15" x2="10" y2="9"></line><line x1="14" y1="15" x2="14" y2="9"></line></svg>
          إيقاف النشر (تحويل لمسودة خاصة)
        `;
      }} else {{
        ...
        badge.innerHTML = `
          ...
          مسودة (غير منشور)
        `;
        desc.innerText = 'هذا القالب مسودة خاصة بك كمسؤول فقط، ولن يظهر لأي طالب حتى تضغط على نشر.';
        toggleBtn.className = 'btn btn-admin btn-sm';
        toggleBtn.style.color = '#fff';
        toggleBtn.style.borderColor = '';
        toggleBtn.innerHTML = `
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"></path><polyline points="16 6 12 2 8 6"></polyline><line x1="12" y1="2" x2="12" y2="15"></line></svg>
          نشر هذا القالب للطلاب الآن
        `;
      }}
    }}
